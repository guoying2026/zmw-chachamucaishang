...
function sendRegCode() {
  ...
  getRegCodeRemainSeconds.value = 60;
  let timerHandle = () => {
    ...
    <div class="fixed px-5 py-4 login-pop-up">
      ...
      <div class="inline-flex flex-col items-center w-full h-auto bg-white text-black px-2 py-4 overflow-hidden login-pop-up-main">
        ...
        <div class="relative inline-flex flex-col w-10/12">
          <input class="w-full h-10 px-2 py-1 mt-6 text-sm md:text-base login-pop-up-main-input-item" placeholder="请输入您的手机号" ref="phoneNumberInputRef" v-model="loginPhoneNumber" />
          <span v-if="isShowPhoneFieldTips && loginPhoneNumber.trim() === ''" class="absolute -bottom-5 left-3 text-xs login-poop-up-main-input-need-field-tips">请输入正确的手机号</span>
        </div>
        <div class="relative inline-flex flex-col w-10/12">
