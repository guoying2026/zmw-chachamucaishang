...
<template>
  <div class="fixed inline-flex flex-row justify-center items-center w-full bg-black py-2 z-10 header">
    ...
    <!-- 搜索框 -->
    <div class="relative inline-flex md:justify-evenly items-center ml-2 md:p-1 md:ml-4 md:border md:border-solid md:rounded-lg search-box search-input">
      <input class="w-full md:w-60 px-5 py-1 md:px-0 md:py-0 md:pr-6 text-xs md:text-sm md:bg-transparent text-inherit md:text-white rounded-2xl md:rounded" type="text" placeholder="请输入企业名、人名等关键词查询" ref="searchTextRef" v-model="searchInputText" @keyup.enter="searchButtonHandle" />
