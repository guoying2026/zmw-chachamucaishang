...
<template>
  ...
  <div class="fixed inline-flex flex-row justify-center items-center w-full bg-black py-2 z-30 header">
    ...
    <div v-if="userInfoStore && typeof userInfoStore.getUserId() == 'string' && userInfoStore.getUserId()" @click.stop="isShowUserInfoPopup = !isShowUserInfoPopup" :class="(isSearchInputFocusing ? 'hidden md:inline-block' : 'inline-block') + ' text-sm md:text-base font-medium whitespace-nowrap px-1 py-0.5 mx-1 md:ml-10 rounded cursor-pointer'">
