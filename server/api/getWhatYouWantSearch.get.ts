export default defineEventHandler(async (e) => {
  try {
    ...
    let uniqueArr = [...new Set(originArr)]
    ...
    let nameArr = name.trim().replaceAll('  ', ' ').split(' ')

    // 如果没有传入name参数，则随机取4个，并且不重复
    if (!name || typeof name != 'string' || name.trim().length == 0 || nameArr.length == 0) {
      let ret = [] as string[]
      for (let i = 0; i < size; i++) {
        let index = Math.floor(Math.random() * uniqueArr.length)
        let item = uniqueArr[index]
        // 如果随机取的已经出现在数组里，则重新抽取
    ...
    if (query.offset && !isNaN(Number(query.offset))) {
      offset = Number(query.offset) as number
    }

    let ret = [] as string[]
    nameArr.forEach(name => {
      uniqueArr.forEach((item: string, index: number) => {
    ...
    if (ret1.length < size) {
      ...
      ret1 = ret1.concat(ret.slice(0, offset))
    }
