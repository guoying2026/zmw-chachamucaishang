...
<template>
  <div class="inline-flex flex-col w-screen h-screen contain">
    ...
    <div class="fixed top-0 z-50 inline-flex flex-row justify-between items-center w-full px-2 pt-6 search-input-box">
      <input class="w-10/12 md:w-11/12 h-8 text-sm pl-6 pr-5 text-black search-input" type="search" placeholder="请输入企业名、人名等关键词查询" ref="searchTextRef" v-model="searchInputText" @focus="scrollGenerateSearchInputWordBox" @change="scrollGenerateSearchInputWordBox" @keyup="scrollGenerateSearchInputWordBox" @keyup.enter="gotoSearch" />
      <!-- 搜索图标 -->
